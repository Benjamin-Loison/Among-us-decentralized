...
void InGameUI::displayPlayer(Player &player, QPainter *painter, bool showGhost, quint16 forceX, quint16 forceY, quint64 time)
{
	...
	else if ( !player.isMoving && time - player.startMoveAt > 1000) {
...
void InGameUI::displayPlayers(QPainter* painter, quint64 time)
{
    ...
    for (Player* player : players)
        displayPlayer(*player, painter, false, 0, 0, time);
    sort(players.begin(), players.end(), [](const Player *a, const Player *b)
          {
              if (a->y != b->y)
                  return a->y < b->y;
    ...
    for (Player* player : players)
        displayPlayer(*player, painter, true, 0, 0, time);
}
